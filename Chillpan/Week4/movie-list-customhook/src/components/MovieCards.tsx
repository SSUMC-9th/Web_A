import { useState } from "react";
import { useNavigate } from "react-router-dom";
import type { Movie } from "../types/movie";

interface MovieCardProps {
  movie: Movie;
}

export default function MovieCards({ movie }: MovieCardProps) {
  const [isHovered, setIsHovered] = useState(false);
  const navigate = useNavigate();

  const handleClick = () => {
    navigate(`/movies/${movie.id}`);
  };

  return (
    <div
      //sm(보통 600px) 이상에서의 제어
      className="relative flex flex-col sm:flex-row gap-2 sm:gap-4 p-2 sm:p-4 bg-gray-100 rounded-lg transition-all duration-300 hover:shadow-lg cursor-pointer"
      onMouseEnter={(): void => setIsHovered(true)}
      onMouseLeave={(): void => setIsHovered(false)}
      onClick={handleClick}
    >
      <img
        src={`https://image.tmdb.org/t/p/w200${movie.poster_path}`}
        alt={`${movie.title} 영화의 이미지`}
        className="w-full sm:w-32 h-32 sm:h-48 object-cover rounded"
      />

      {isHovered && (
        <div className="flex-1 flex flex-col justify-center">
          <h2 className="text-sm sm:text-lg font-bold mb-1 sm:mb-2">
            {movie.title}
          </h2>
          <p className="text-xs sm:text-sm text-gray-600 line-clamp-2 sm:line-clamp-4">
            {movie.overview}
          </p>
          <div className="mt-1 sm:mt-2 text-xs text-gray-500">
            <p>개봉일: {movie.release_date}</p>
            <p>평점: {movie.vote_average}/10</p>
          </div>
        </div>
      )}
    </div>
  );
}
