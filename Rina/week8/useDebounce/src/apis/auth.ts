...
export const getMyInfo = async () => {
    const {data} = await api.get("/v1/users/me", {withAuth: true});
    return data;
}
...
export const postGoogleLogin = async (credential: string)=> {
    const {data} = await api.post<ResponseSigninDto>('/v1/auth/google', {
        credential,
    });
    return data;
}